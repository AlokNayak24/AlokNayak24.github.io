...
export default function Header() {
  ...
  const menuVariants: Variants = {
    open: { clipPath: "circle(1500px at 90% 5%)", transition: { type: "spring", stiffness: 20, restDelta: 2 } },
    closed: { clipPath: "circle(0px at 90% 5%)", transition: { type: "spring", stiffness: 400, damping: 40 } },
  };
  ...
  const navItems = NAV_SECTIONS.filter((s) => s.id !== "hero");
          ...
          <div className="glass-panel w-full max-w-7xl rounded-[2rem] flex items-center justify-between px-6 py-4 shadow-xl">
            ...
            <nav className="hidden lg:flex flex-1 justify-center">
              <ul className="flex space-x-6">
                {navItems.map((item) => (
                  <motion.li key={item.id} className="relative group text-sm font-medium text-muted-foreground transition-colors">
                    <a onClick={() => handleScrollTo(`#${item.id}`)} className="cursor-pointer hover:text-foreground">
                      {item.label}
                    </a>
                    <motion.span
            ...
            <div className="flex items-center gap-2">
              <a
                href={siteData.identity.resumeUrl}
                download
                className="hidden md:inline-flex items-center gap-2 rounded-full px-4 py-2 text-xs font-bold glass-panel border border-foreground/10 hover:border-primary/40 hover:text-primary transition-all"
              >
          ...
          <AnimatePresence>
            {isMobileMenuOpen && (
              ...
              >
                ...
                >
                  {navItems.map((item) => (
                    <motion.li key={item.id} {...({ variants: itemVariants } as MotionProps)}>
                      <a
                        onClick={() => handleScrollTo(`#${item.id}`)}
                        className="text-3xl font-bold text-muted-foreground hover:text-primary hover:tracking-wider transition-all cursor-pointer"
                      >
                        {item.label}
                      </a>
                    </motion.li>
